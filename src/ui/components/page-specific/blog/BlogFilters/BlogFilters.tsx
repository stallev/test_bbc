...
const BlogFilters: React.FC<BlogFiltersComponentsProps> = ({
  ...
  const isResetBtnActive = (Object.keys(filters) as (keyof BlogFiltersProps)[]).some(
    filterKey => filters[filterKey] !== DEFAULT_BLOG_POSTS_FILTER_STATE[filterKey]
  );
  ...
  const filterAndSortPosts = (
    ...
    const filteredPosts = posts.filter(post => {
      ...
      if (topics !== ALL_TAXONOMY_ITEMS_NAME && !post.topics.some(topic => topic === topics)) {
        return false;
      }
    ...
    const sortedPosts = filteredPosts.sort((a, b) => b.date.timestamp - a.date.timestamp);
  ...
  return (
    <div className={styles['blog-filters']}>
      ...
      {isResetBtnActive && (
        <Button className={styles['blog-filters__reset-btn']} type="primary" onClick={resetFilters}>
          <Text textType="span">{translate('reset')}</Text>
          <Icon className={styles['blog-filters__reset-btn-icon']} iconName="rightArrow" />
        </Button>
      )}
    </div>
  );
