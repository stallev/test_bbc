...
import { CustomDatePicker, Text, Icon, Button } from '@/ui/components/ui-kit';
...
const SermonFilters: React.FC<SermonsFiltersComponentProps> = ({
  ...
  const isResetBtnActive = (Object.keys(filters) as (keyof SermonsFiltersProps)[]).some(
  ...
  const filterAndSortSermons = (
    ...
    const filteredSermons = sermons.filter(sermon => {
      ...
      if (biblebooks !== 'all' && !sermon.biblebooks.some(item => item.id === biblebooks)) {
        return false;
      }

      if (preachers !== 'all' && !sermon.preachers.some(item => item.id === preachers)) {
        return false;
      }

      if (topics !== 'all' && !sermon.topics.some(item => item.id === topics)) {
        return false;
      }
      ...
      <div className={styles['sermons-filters__categories']}>
        ...
        {isResetBtnActive && (
          ...
          >
            ...
            <Icon className={styles['sermons-filters__reset-btn-icon']} iconName="rightArrow" />
          </Button>
        )}
      </div>
    </div>
