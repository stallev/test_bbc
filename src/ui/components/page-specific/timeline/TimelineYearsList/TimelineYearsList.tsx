...
const TimelineYearsList: React.FC<TimelineYearsListProps> = ({ data }) => {
  const yearsList = data.map((item, index) => (
    <TimelineYear key={index} year={item.year} eventsList={item.eventsList} />
  ));
  return <div className={styles['timeline-year-list']}>{yearsList}</div>;
};
