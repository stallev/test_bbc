...
const TimelineEvent: React.FC<TimelineEventProps> = ({
  ...
  return (
    <div className={styles['timeline-event']}>
      <p className={styles['timeline-event__date']}>{eventDate}</p>
      <Link
        href={postLink}
      ...
      <div className={styles['timeline-event__info']}>
        <Link
          ...
          tabIndex={0}
        >
          <h2 className={styles['timeline-event__title']}>{title}</h2>
        </Link>

        <p className={styles['timeline-event__description']}>{stripHtmlTags(excerpt)}</p>

        <div className={styles['timeline-event__meta']}>
          <Link href={postLink} className={styles['timeline-event__read-more-link']} tabIndex={0}>
