...
const ChurchContactsInfo = ({ locale, isPageContentType = false }: ChurchContactsInfoProps) => {
  ...
  return (
    <div className={`${styles["church-contacts-info"]} ${isPageContentType ? styles["church-contacts-info--page-content"] : ''}`}>
      <a
        href={mapPageAddress}
        target='_blank'
      ...
      <a className={styles["church-contacts-info__email-link"]} href={`mailto:${ContactsInfo.email}`}>{ContactsInfo.email}</a>
      <a className={styles["church-contacts-info__phone-link"]} href={`tel:${ContactsInfo.phoneValue}`}>{ContactsInfo.phonePrint}</a>

      <Text
        textType="span"
        className={styles["church-contacts-info__work-hours"]}
      >
