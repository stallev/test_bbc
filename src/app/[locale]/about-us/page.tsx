...
export async function generateMetadata(props: PagePathProps): Promise<Metadata> {
  ...
  const pageSlug =
    locale === i18n.defaultLocale ? PagesSlugs.AboutUs[i18n.defaultLocale] : PagesSlugs.AboutUs.ru;

  return await getPageSeoData({ pageSlug, locale, pagePath: RoutePath.AboutUs });
}
...
export default async function AboutUs(props: PagePathProps) {
  ...
  const { timeline_data, mission_data } = await PageContentDataApi.getAboutUsData(pageSlug);
