...
export async function generateMetadata(props: PagePathProps): Promise<Metadata> {
  ...
  const pageId =
    locale === i18n.defaultLocale
      ? PagesIDs.WorshipMinistry[i18n.defaultLocale]
      : PagesIDs.WorshipMinistry.ru;
...
export default async function WorshipMinistry(props: { params: Promise<{ locale: Locale }> }) {
  const params = await props.params;
  const { locale } = params;
