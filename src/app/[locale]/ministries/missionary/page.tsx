...
export async function generateMetadata(props: PagePathProps): Promise<Metadata> {
  ...
  const pageId =
    locale === i18n.defaultLocale
      ? PagesIDs.MissionaryMinistry[i18n.defaultLocale]
      : PagesIDs.MissionaryMinistry.ru;
...
export default async function MissionaryMinistry(props: { params: Promise<{ locale: Locale }> }) {
  const params = await props.params;
  const { locale } = params;
