...
export const getServiceTimeDifference = (event: EventParameterProps): EventData => {
  const eventDurationMillisecondsCount =
    event.durationTime.hour * MillisecondsCount.inHour +
  ...
  const addedMillisecondsCount =
    addedDaysCount * MillisecondsCount.inDay +
    event.timeHours * MillisecondsCount.inHour +
    eventDurationMillisecondsCount +
    pacificOffset * MillisecondsCount.inHour;

  const assumedDifferenceEndTimeMark =
    todayStartTimeMark + addedMillisecondsCount - currentTimeMark;

  const differenceEndTimeMark =
    assumedDifferenceEndTimeMark > 0
